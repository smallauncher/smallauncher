	...
	match command.as_str() {
		"-a" | "--authenticate" => {
			match smallauncher_lib::auth::auth_microsoft() {
				Ok(auth) => {
					match smallauncher_lib::auth::save(&auth_path, &auth) {
		...
		"-l" | "--list" => {
			match launch::list_versions(&game_path) {
				Ok(list) if !list.is_empty() => {
					for version in list {
						println!("{version}");
					}
				},
				Ok(_) => println!("No installed versions found."),
				Err(error::Error::Io(e)) if e.kind() == std::io::ErrorKind::NotFound => {
		...
		"-la" | "--list-all" => {
			match launch::list_all_versions() {
				Ok(versions) => {
